%poly2017 generates the polycon basis functions.
syms x y 
...
sixy = vpa(sixy,6);
    ...
    for k = 1:n
        ...
        if degsi(k)*degsi(km1) == 4
            ...
            emin = 1e6;             
            for s = 1:lx
                e1(s) = (xvk(s) - vertest(1,k))^2 + (yvk(s) - vertest(2,k))^2;
                if e1(s) < emin
                    emin = e1(s);
                    smin = s;
                end
            ...
            if ep == 1 %There are two eip on the absolute line
                ...
                elseif ayy == 0
                    ...
                    eip(2,3*k-1) = -axy + disc;
                    eip(2,3*k) = -axy - disc;
                    eip(3,3*k-1) = 2*ayy;
                    eip(3,3*k) = 2*ayy;                
                end
            end
            if eip(2,3*k) ~= real(eip(2,3*k)) | eip(3,3*k) ~= real(eip(3,3*k))
                ee = eip(:,3*k);
                eip(:,3*k) = eip(:,3*k-2);                
                eip(:,3*k-2) = ee;                   
            end
        else
            if degsi(k) == 1                                    %Line 150
...
sino = zeros(2,n);        
    for k = 1:n
        ...
        if degsi(k) == 1
            ...
            pb2 = (vert(2,kp1)-vert(2,k))*y;                    %Line 250
...
%The eip is on the absolute line.
            ...
            if esum == 0
%Adjacent identical parabolas must be addresed 
                syms uu vv
...
cQ = coeffs(Q);
    if cQ == Q
        ...
        for s = 1:mm
            c1 = coeffs(cQ(s));
            if  length(c1) == 1 & c1 == cQ(s);              %Line 400
                c1 = double(c1);
                if abs(c1) < 1e-8
                    cQ(s) = 0*cQ(s);
                end
...
%sitdpp is the ratio of sit to p12 near sino on side k.
            ...
            for s = 1:4
                t = 5-s;
                if abs(sx) > abs(sy)
                    x1(s) = sino(1,k) + 1e-4*t*sx;
                    s1 = subs(sixy(k),x,x1(s));
                    y1c = double(solve(s1,y));
                    if length(y1c) == 2
            de1 = abs(y1c(1) - sino(2,k)); de2 = abs(y1c(2) - sino(2,k));   %LINE 500
