...
elseif rep == 10
    ...
    sixy(1) =  4 + y;
    sixy(2) = 3 - x;
    sixy(3) = 1 - x*y + .01*x^2 +.001*y^2;                           %Line 85
    sixy(4) = 3 - y;                
    sixy(5) = 4 + 7*x + y;	
    degsi = [1 1 2 1 1];
    vertest = [0 3 3 .3333 -1;-4 -4 .3333 3 3];
end 
    for k = 1:n
        km1 = k-1;
        if k == 1
...
%We now determine the ratio of sit to p1*p2 at the side node
       if abs(double(ax)) < abs(double(ay))
            ...
            dpkp1 = diff(pkp1,y);
            dpkp1si = double(subs(dpkp1,y,ysi));        %line 517
       end                                              
